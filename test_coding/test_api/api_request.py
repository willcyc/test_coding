...
def request_api_post_form(url, headers, data):
    ...
    response = request.post(url, headers=ast.literal_eval(headers), data=data_, timeout=(5, 5))
...
def request_api_post_json(url, headers, data):
    ...
    response = request.post(url, headers=ast.literal_eval(headers), json=data_, timeout=(5, 5))
...
def request_api_get_form(url, headers, payload):
    ...
    response = request.get(url, headers=ast.literal_eval(headers), params=payload, timeout=(5, 5))
    response_time = response.elapsed.total_seconds()
    # return response.text, response_time
    return response, response_time
...
def request_api_get_json(url, headers, payload):
    ...
    response = request.get(url, headers=ast.literal_eval(headers), params=payload_, timeout=(5, 5))
    response_time = response.elapsed.total_seconds()
    # return response.text, response_time
    return response, response_time
